...
# by Sahith Reddy Thummala (Sahith59). Design credit: Andrew Grant.
#
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    request = requests.post('https://api.github.com/graphql', json={'query': query, 'variables': variables}, headers=HEADERS)  # do not use simple_request() here, so the cache file can be saved before raising an Exception
    if request.status_code == 200:
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    ...
    Forks are excluded from the LOC count (isFork: false), so upstream histories are never walked.
    Queries 60 repos at a time to stay under the API's timeout.
    """
...
if __name__ == '__main__':
    """
    Sahith Reddy Thummala (Sahith59), 2026
